...
| **MMPI / СМИЛ / ММИЛ** | 3 валид. + 10 базисных (T-баллы) + код профиля + тип + валидность |
...
Интерпретация СМИЛ/ММИЛ построена на работах:
- **Собчик Л.Н.** — «СМИЛ (MMPI). Стандартизированный метод исследования личности» (3-е изд., 2020).
  Используем модифицированные (поведенческие) названия шкал вместо клинических:
  не «паранойя», а «ригидность»; не «шизофрения», а «индивидуалистичность».
- **Березин Ф.Б., Мирошников М.П., Соколова Е.Д.** — «Методика многостороннего исследования
  личности» (3-е изд. — совместная интерпретация ММИЛ + 16PF Кеттелла).
- **Правила интерпретации:** по целостному профилю и коду профиля, а не по отдельным шкалам.
  Код надёжен при T > 69 (СМИЛ/ММИЛ) или T > 64 (MMPI-2), разница между пиками ≥ 5 T.
- **5 сфер анализа профиля СМИЛ:** личностные черты → проблемные зоны → ведущие
  потребности → когнитивные искажения → профпригодность.

Дополнительные источники по 16PF, DISC, HOLLAND, Big Five — в `docs/sources.md`.
